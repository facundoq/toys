...
public class ChanceToHitStatistic extends DerivedStatistic {
	...
	public static ChanceToHitStatistic newChanceToHit(float baseValue, Attribute speed, Attribute dexterity, Skill weapons) {
	...
	public ChanceToHitStatistic(String name, float baseValue, ArrayList<Statistic> dependingStatistics) {
		super(name, baseValue, dependingStatistics);
	}

	@Override
	public float calculate() {
		float value = this.getBaseValue() + (this.getValueOfDependingStatistic("Weapons") / 3 + this.getValueOfDependingStatistic("Speed") / 10 + this.getValueOfDependingStatistic("Dexterity") / 3);
