...
public class StaminaCostPerHitStatistic extends DerivedStatistic {
	...
	public static StaminaCostPerHitStatistic newStaminaCostPerHit(float baseValue, Attribute endurance, Attribute dexterity, Skill weapons) {
		...
		return new StaminaCostPerHitStatistic("StaminaCostPerHit", baseValue, statistics);
	}

	public StaminaCostPerHitStatistic(String name, float baseValue, ArrayList<Statistic> dependingStatistics) {
		super(name, baseValue, dependingStatistics);
	}

	@Override
	public float calculate() {
		float value = this.getBaseValue() - (this.getValueOfDependingStatistic("Endurance") / 3 + this.getValueOfDependingStatistic("Dexterity") / 10 + this.getValueOfDependingStatistic("Weapons") / 3);
