...
import struct 
import os
def seq_to_img(seq_filepath,prefix,output_folderpath):
    if not os.path.exists(output_folderpath):
        os.makedirs(output_folderpath)
    def read_header(ifile):
        feed = ifile.read(4) # "0xFEED"
        norpix = ifile.read(24) # norpix sequence (24 bytes)
        ...
        # skipping the rest
        ifile.read(432)
        image_ext = {100:'raw', 102:'jpg',201:'jpg',1:'png',2:'png',103:'jbrgb',101:'brgb8'}
    ...
    ifile = open(seq_filepath, 'rb')
    params = read_header(ifile)
    bytes = open(seq_filepath, 'rb').read()
    ...
    for i in range(0, params['num_frames']-1):
        ...
        filename='%simg%05d.%s' % (prefix,i,params['ext'])
        tmp_file = os.path.join(output_folderpath,filename)
