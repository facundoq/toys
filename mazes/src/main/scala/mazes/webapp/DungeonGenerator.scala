...
class Categorical(var probabilities: List[Float]) extends Distribution {
  ...
  def draw(): Int = {
    ...
    return cumulativeProbabilities.indexWhere(_ < p)
  }
}
...
class SimpleMaze(size: Size, var keepDirectionProbability: Float) extends DungeonGenerator(size) {
  ...
  def generate(): String = {
    ...
    while (visitedOrWallCount < size.count) {
      ...
      val posiblePositions = Direction.values.map(d => p + d.asTuple())

      val validPositionFactor = posiblePositions.map(x => size.isValid(x) toInt)

      val visitedPositionInt = posiblePositions.map(p => visited(p.x)(p.y) toInt)
      val visitedPositionFactor = visitedPositionInt.map(f => 1-f)
      ...
      var positionProbabilities = (validPositionFactor, mantainDirectionFactor, visitedPositionFactor).zipped.map(_ * _ * _)
