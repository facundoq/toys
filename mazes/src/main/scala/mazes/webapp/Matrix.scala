...
class Matrix[T](val array: ArraySeq[ArraySeq[T]],val rows: Int,val columns: Int) 
  ...
  def randomIndex():(Int,Int)= randomIndex(new Random())
  def randomIndex(r:Random):(Int,Int)={
    (r.nextInt(rows), r.nextInt(columns))
  }
  /**
...
object Matrix {
  ...
  def apply[T](size:(Int,Int), default: => T) : Matrix[T] =   
    apply(size._1,size._2,default)
